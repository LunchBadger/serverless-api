...
class ServerlessExecutor {
  ...
  // no need for author, since owner user initiates repo
  ...
  async removeService ({name, author}) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        await commit({msg: `LB: service ${name} removed`, author});
        resolve({msg: 'REMOVED'});
      } catch (err) {
        reject(err);
      }
  ...
  async updateFiles ({name, data, author}) {
    ...
    return dirAsObject.deserialize(data.files).then(() => commit({msg: 'LB: edit', author}));
  }
