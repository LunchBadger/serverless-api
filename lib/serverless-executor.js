...
process.cwd = function () {
  // Dirty hack for serverless
  const res = originalCWD();
  console.log('AAAAA', res, rootFolder);

  if (res.indexOf(rootFolder, 1) >= 0) {
    return res.replace(rootFolder, '');
  }
  return res;
};
...
class ServerlessExecutor {
  async createFromTemplate (config) {
    const servicePath = path.join(rootFolder, config.servicePath);
    const s = this.initiateServerless({});
    s.inputArray = ['create', '--template', 'kubeless-nodejs', '--path', servicePath];
